...
export const FileReplacer = ({ fileName = "", doc, EntityType, EntityID }) => {
	...
	const [typeId, setTypeID] = useState(doc.typeId || doc.documentTypeId);
	const [fileUrl, setFileUrl] = useState(doc.url);
	const [fileId, setFileId] = useState(doc.id || doc.documentId);
	...
	}, [file, fileUrl, setFileUrl, typeId, fileId]);
	return (
		<div>
			<Toaster position="bottom-center" />
			<div className=" relative">
				<label className="flex disabled:cursor-none flex-col z-0 justify-center items-center py-12 px-16 mb-4 bg-[#FCFCFC] hover:bg-gray-50 transition-all duration-300 rounded-lg border-2 border-gray-300 border-dashed cursor-pointer">
