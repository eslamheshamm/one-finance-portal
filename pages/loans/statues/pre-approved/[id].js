...
import { FileUploader } from "../../../../components/Atomics/Files/FileUploader";
...
const AcceptedLoan = () => {
	...
	const [loanInstallment, setLoanInstallment] = useState(null);
	const [isLoanScheduleCreated, setIsLoanScheduleCreated] = useState(false);
	...
	const CreateLoanSchedule = () => {
		apiClient
			.post("/api/Loan/CreateLoanSchedule", { loanId: loanId })
			.then((res) => {
				if (res.data.isSuccess) {
					setIsLoanScheduleCreated(true);
				}
			})
			.catch((error) => {
				console.error(error);
	...
	const GetLoanInstallment = () => {
		const loader = toast.loading("جاري تحميل جدول الأقساط..");
		apiClient
			.post("/api/Loan/GetInstallmentSchedule", { loanId: loanId })
			.then((res) => {
				toast.dismiss(loader);
				setLoanInstallment(res.data.installment);

				if (!res.data.isSuccess) {
					toast.error("لقد حدث خطأ في إنشاء جدول الأقساط.");
				}
			})
			.catch(() => {
				toast.dismiss(loader);
	...
	const GetContractData = () => {
		apiClient
			.post("/api/Loan/GetContractData", { loanId: loanId })
			.then(({ data }) => {
	...
	const FillContract = () => {
		const loader = toast.loading("يرجي الإنتظار جاري إنشاء العقد..");
		axios
			.post("/api/pdf", {
	...
	useEffect(() => {
		if (loanId && isLoanScheduleCreated) {
			GetLoanInstallment();
		}
	}, [loanId, isLoanScheduleCreated, setIsLoanScheduleCreated]);

	const handleCompleteLoan = (e) => {
		e.preventDefault();
		const loading = toast.loading("جاري إكمال التمويل ..");
		apiClient
			.post("/api/Loan/CompleteLoan", {
				id: loanId,
	...
	return (
		<DashboardLayout>
			...
			{customerInfo && (
				<section className="w-10/12 mx-auto shadow-sm bg-white rounded-3xl">
					<div className="flex items-center justify-start  pt-8 bg-[#FFC662]  rounded-3xl">
						<Image
						...
						<div className="mr-4">
							...
							<p className="text-4xl text-white">
								التمويل مقبول, يرجي رفع المستندات المطلوبة...
							</p>
						</div>
					</div>
					{loanInstallment && (
						<div
							className=" rounded-3xl  mt-12 w-11/12 hidden print:w-full print:font-Cairo  print:block"
							dir="rtl"
							ref={componentRef}
						>
							<table className=" text-center my-12 table-fixed  w-11/12 ">
								<thead className="border-white text-center font-medium ">
									<tr>
										<th className="px-1 pt-6 pb-4 font-semibold">القسط </th>
										<th className="px-1 pt-6 pb-4 font-semibold">التاريخ </th>
										<th className="px-1 pt-6 pb-4 font-semibold">
											الرصيد الافتتاحي
										</th>
										<th className="px-1 pt-6 pb-4 font-semibold">قيمة القسط</th>
										<th className="px-1 pt-6 pb-4 font-semibold">الفائدة</th>
										<th className="px-1 pt-6 pb-4 font-semibold">اصل المبلغ</th>
										<th className="px-1 pt-6 pb-4 font-semibold">
											الرصيد الختامي
										</th>
									</tr>
								</thead>
								<tbody className="text-center text-sm overflow-x-auto ">
									{loanInstallment &&
										loanInstallment.map((item, index) => {
											const formatedDate = moment(item.installmentDate).format(
												"l"
											);
											return (
												<tr
													className=" bg-white text-center text-[#6E7191]"
													key={index++}
												>
													...
													<td className="px-2 py-4 text-center">
														<div className="flex items-center text-center px-8">
															<p className="text-center">{item.interest}</p>
														</div>
													</td>
													<td className=" py-4 text-center">
														<div className="flex items-center text-center px-8">
															<p className="text-center">{item.principal}</p>
														</div>
													</td>
													<td className=" py-4 text-center">
														<div className="flex items-center text-center px-8">
					...
					<div className={"pt-8 bg-white px-12  rounded-3xl"}>
						{comment && (
							<div className=" mb-6">
					...
					<div className="mt-8  px-12 flex items-center gap-6">
						<button
							disabled={loadingContractdata}
							onClick={() => {
								FillContract();
							}}
							className="px-8 py-6 bg-[#FFC662] rounded-2xl flex items-center  font-semibold text-xl"
						>
							<span>{DowloanIcon}</span>
							<span className="mr-4">إنشاء العقد</span>
						</button>

						<button
							disabled={!isContractCreated}
							className={`disabled:bg-gray-500 bg-green-600 px-8 py-6 rounded-2xl flex items-center  font-semibold text-xl text-white`}
							onClick={() => {
								GetContractPdf();
							}}
						>
							<span>{DowloanIcon}</span>
							<span className="mr-4">تحميل العقد</span>
						</button>
					</div>
					<div className={"pt-4 pb-8  px-12  rounded-3xl"}>
						<h5 className="my-8 font-bold text-xl">
							تحميل المستندات ( الخطوة الأولي )
						</h5>

						<div className="mt-8 text-black grid grid-cols-4  items-center gap-4 ">
							{loanDownloadDocs &&
								loanDownloadDocs.map((doc, index) => {
									return (
										<button
											key={index++}
											className="px-8 py-6 bg-[#FFC662]	  rounded-2xl text-sm"
										>
											<a
												href={doc.url}
							...
							>
								<div className="px-8 py-6 bg-[#FFC662] rounded-2xl flex items-center  font-semibold text-xl ml-6 ">
									<span>{DowloanIcon}</span>
									<span className="mr-4">
										{isLoanScheduleCreated
											? "جدول الاقساط"
											: "جاري إنشاء جدول الاقساط"}
									</span>
								</div>
							</button>
						</div>
						<h5 className="my-8 font-bold text-xl">
							رفع المستندات ( الخطوة الثانية )
						</h5>
						...
						<div className="mt-12 flex items-center  justify-end">
							<button
								onClick={(e) => {
									handleCompleteLoan(e);
								}}
								className="px-20 py-6 bg-[#343434] rounded-full flex items-center text-white  font-bold text-xl"
							>
								<span>إرسال </span>
							</button>
						</div>
