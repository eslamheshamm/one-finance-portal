				...
				<div className="flex items-center">
					<Link
						href={
							status === "PreApproved"
								? {
										pathname: "/loans/statues/pre-approved/[id]",
										query: { id: id },
								  }
								: status === "Pending"
								? {
										pathname: "/loans/statues/pending/[id]",
										query: { id: id },
								  }
								: status === "RequestForEdit"
								? {
										pathname: "/loans/statues/edit/[id]",
										query: { id: id },
								  }
								: status === "rejected"
								? {
										pathname: "/loans/statues/rejected/[id]",
										query: { id: id },
								  }
								: {
										pathname: "/loans/partnerships/queue",
								  }
						}
					>
						<button
							className={classNames(
								...
								status === "PreApproved" && "bg-[#CCF5E0] text-[#00CC67]",
								status === "Approved" && "bg-[#CCF5E0] text-[#00CC67]",
								status === "RequestForEdit" && "bg-[#eb502a] text-[#ffffff]",
								status === "Pending" && "bg-[#eb502a] text-[#ffffff]",
								status === "Rejected" &&
									"bg-[#FF0000] bg-opacity-30 text-[#FF0000]"
							)}
						>
							{status === "PreApproved" && "مقبول مبدائياً"}
							{status === "Approved" && "مقبول"}
							{status === "Pending" && "تعديل البيانات"}
							{status === "RequestForEdit" && "تعديل المستندات"}
