...
const RiskManagerQueue = () => {
	...
	const GetRequestedLoansRisk = () => {
		apiClient
			.post("/api/Loan/GetAllRequestedLoans")
			.then(({ data }) => {
								...
								<table className="w-full  ">
									<thead className="border-white text-right font-medium text-lg  bg-white">
										<tr>
											...
											<th className="px-6 pt-6 pb-4 font-semibold">الموظف</th>
...
export default RiskManagerQueue;
		...
		<tr className="bg-white text-center text-[#6E7191]">
			...
			<td className="px-6 py-4">
				<div className="flex items-center">
					<Link
						href={{
							pathname: "/risk/manager/statues/approved/[id]",
							query: { id: id },
						}}
					>
						<button
