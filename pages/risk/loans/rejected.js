...
const RiskRejectedLoans = () => {
	const [q, setQ] = useState("");
	const [searchParam] = useState(["customer"]);
	const [customersData, setCustomersData] = useState([]);
	const [loading, setLoading] = useState(true);
	const GetRejectedLoansRisk = () => {
		apiClient
			.post("/api/Loan/GetRejectedLoans")
			.then(({ data }) => {
				if (data.isSuccess) {
					setLoading(false);
					setCustomersData(data.rejectedLoans);
				}
			})
			.catch((err) => console.log(err));
	};
	useEffect(() => {
		GetRejectedLoansRisk();
	}, []);
...
export default RiskRejectedLoans;
		...
		<tr className="bg-white text-center text-[#6E7191]">
			...
			<td className="px-6 py-4">
				<div className="flex items-center">
					<Link
						href={{
							pathname: "/risk/loans/statues/rejected/[id]",
							query: { id: id },
						}}
					>
						<button
