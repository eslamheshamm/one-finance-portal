...
const RiskPendingCustomers = () => {
	...
	const fetchData = () => {
		setLoading(true);
		apiClient
			.post("/api/Loan/GetCompletedLoans")
			.then((res) => {
				setCustomersData(res.data.completedLoans);
				console.log(res.data.completedLoans, "data");
				setLoading(false);
			})
			.catch((error) => {
				console.log(error);
					...
					>
						<button
							className={classNames(
								...
								status === "Completed" && "bg-[#CCF5E0] text-[#00CC67]",
								status === "Edited" && "bg-[#FFF8F2] text-[#FF7500]",
								status === "Updated" && "bg-[#FFF8F2] text-[#FF7500]"
							)}
						>
							{status === "Completed" && "مقبول"}
							{status === "Edited" && "تم التعديل"}
							{status === "Updated" && "تم التعديل"}
						</button>
					</Link>
				</div>
			</td>
