...
const RejectedLoan = () => {
	...
	const [refueseReasson, setRefueseReasson] = useState("");
	const GetLoanData = () => {
		apiClient
		...
		<DashboardLayout>
			...
			{customerInfo && (
				<section className="w-10/12 mx-auto shadow-md pb-8 rounded-3xl bg-white">
					<div>
						<div className="flex items-center justify-start  pt-8 bg-[#FF4D4D]  rounded-3xl  px-8 ">
							<Image
								src="/loans/rejected.png"
								alt="Almasria Logo"
								width={164}
								height={228}
							/>
							<div className="mr-4">
								<h2 className="text-7xl mb-3">عذراً!</h2>
								<p className="text-4xl text-white">التمويل مرفوض...</p>
							</div>
						</div>

						<div className={"p-8  border-gray-100 rounded-2xl  "}>
					...
					<div className="mt-12 flex items-center justify-end px-12">
						<button
							onClick={() => {
								router.back();
							}}
							className="px-12 rounded-full py-4 bg-[#6E7191] flex items-center text-white  font-bold text-xl mr-8"
						>
							<span>الرجوع </span>
						</button>
					</div>
