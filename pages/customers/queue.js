					...
					<Link
						href={
							status === "Approved Customer"
								? {
										pathname: "/customers/statues/accepted/[id]",
										query: { id: id },
								  }
								: status === "Pending Customer"
								? {
										pathname: "/customers/statues/pending/[id]",
										query: { id: id },
								  }
								: status === "Rejected Customer"
								? {
										pathname: "/customers/statues/rejected/[id]",
										query: { id: id },
								  }
								: {
										pathname: "/customers/queue",
								  }
						}
					>
						<button
...
const FilterIcon = (
	...
	>
		...
		<path
			...
			strokeLinecap="round"
		/>
		<circle cx="17.5" cy="17" r="3" stroke="white" strokeWidth="2" />
		<circle cx="6.5" cy="7" r="3" stroke="white" strokeWidth="2" />
	</svg>
);
const Person = (
	<svg
