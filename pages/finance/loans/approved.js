...
const FinanceApprovedLoans = () => {
	...
	const GetApprovedLoansFinance = () => {
		setLoading(true);
		apiClient
			.post("/api/Loan/GetAcceptedLoansByDate", {
				fromDate: range[0].startDate,
				toDate: range[0].endDate,
			})
			.then((res) => {
				if (res.data.isSuccess) {
					setCustomersData(res.data.acceptedLoans);
					setLoading(false);
				}
			})
	...
	return (
		<DashboardLayout>
			<div className="w-10/12 mx-auto bg-white p-8 rounded-3xl shadow-lg">
				<div className="w-full grid grid-cols-4 gap-6 items-end">
					<div className=" w-full space-y-3 col-span-3 flex flex-col">
						<div className="font-bold text-xl">تاريخ البحث</div>
						<div className="calendarWrap">
							<div className="flex justify-end w-full">
								<button
									onClick={() => setOpen((open) => !open)}
									className={classNames("inputBox ", buttonClass)}
								>
									{` من ${moment(range[0].startDate).format(
										"D-M-YYYY "
									)} إلي  ${moment(range[0].endDate).format("D-M-YYYY")}`}
								</button>
							</div>
								...
								{open && (
									<div dir="ltr">
										<DateRangePicker
											...
											className="calendarElement text-gray-400s"
											color="gray"
										/>
									</div>
								)}
			...
			{/* <div className="w-10/12 mx-auto flex justify-end">
				<div className="calendarWrap flex justify-end  w-full">
					<div className="flex justify-end w-full">
						<button
							onClick={() => setOpen((open) => !open)}
							className="inputBox w-full p-4 bg-gray-400 rounded-3xl"
						>
							{` من ${moment(range[0].startDate).format(
								"M-D-YYYY "
							)} إلي  ${moment(range[0].endDate).format("M-D-YYYY")}`}
						</button>
					</div>
...
export default FinanceApprovedLoans;
		...
		<tr className="bg-white text-center text-[#6E7191]">
			...
			<td className="px-6 py-4">
				<div className="flex items-center">
					<Link
						href={{
							pathname: "/finance/loans/statues/approved/[id]",
							query: { id: id },
						}}
					>
						<button
